...
<body>
    ...
    <div class="contaier-fluid text-center">
      <div class="row">
        Improving Human Lives and Environmental Conditions</div>
      <div class="row">Institute for Social and Environmental Research – Nepal (ISER-N)</div>
      <div class="row"></div>
    </div>
    ...
    <div class="container-fluid">
      <div class="container text-center" style="margin-top: 20px; margin-bottom: 20px;">
          ...
          <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
              @foreach($data['posts_latest_newss'] as $posts_latest_news)
                  <div class="col">
                      <div class="card h-100">
                          @if ($posts_latest_news->getFirstMediaUrl())
                              <img src="{{ $posts_latest_news->getFirstMediaUrl() }}" class="card-img-top .img-fluid img-thumbnail" alt="ISER-N">
                          @endif
                          <div class="card-body">
                              <a href="{{ route('posts.show', ['slug' => $posts_latest_news->slug]) }}" style="text-decoration: none; color: inherit;">
                                  <h5 class="card-title">{{ $posts_latest_news->title}}</h5>
                              </a>
                              <p class="card-text">{{ $posts_latest_news->excerpt() }}</p>
                              <a href="{{ route('posts.show', ['slug' => $posts_latest_news->slug]) }}" class="btn btn-primary">Read More</a>
                          </div>
                      </div>
                  </div>
              @endforeach
